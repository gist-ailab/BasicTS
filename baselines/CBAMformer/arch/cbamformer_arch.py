...
class CBAMformer(nn.Module):
    def __init__(self, n_feat, in_ts_len, out_ts_len, seg_len, merge_win_size = 4,
        ...
        self.pad_in_ts_len = ceil(1.0 * in_ts_len / seg_len) * seg_len
        self.pad_out_ts_len = ceil(1.0 * out_ts_len / seg_len) * seg_len
        ...
        self.enc_pos_embedding = nn.Parameter(torch.randn(1, n_feat, self.n_seg, self.d_seg))
        ...
        self.encoder = Encoder(e_layers, merge_win_size, d_seg, n_heads, d_ff, block_depth = 1, \
                                    dropout = dropout,in_seg_num = self.n_seg, factor = factor)
        
        # Decoder
        self.dec_pos_embedding = nn.Parameter(torch.randn(1, n_feat, self.n_seg, self.d_seg))
        self.decoder = Decoder(n_feat, e_layers + 1, self.d_seg, n_heads, d_ff, dropout, \
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs) -> torch.Tensor:
        ...
        dec_in = repeat(self.dec_pos_embedding, 'b ts_d l d -> (repeat b) ts_d l d', repeat = batch_size)
        dec_in = repeat(self.dec_pos_embedding, 'b n_feat d_seg -> (repeat b) n_feat d_seg', repeat=batch_size)
